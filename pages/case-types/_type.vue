<template>
    ...
    <div class="content-body">
            ...
            <template v-else>
                ...
                <div class="form-head d-flex mb-3 mb-lg-5 align-items-start">
                    <div class="mr-auto d-none d-lg-block">
                        <a class="mb-0 text-secondary fs-18 font-w500" href="#">All Case Types </a>
                      ...
                      <div class="modal-dialog modal-lg" role="document">
                          <div class="modal-content">
                              ...
                              <div class="modal-body">
                                  ...
                                  <form @submit.prevent="addCaseType">

                                      <div class="form-row">
                                          <div class="form-group col-md-12">
                                              
                                              <center><label>Type name</label></center>
                                              <input type="text" class="form-control" placeholder="" v-model="type_name">
                                          </div>

                                      </div>

                                      <center>
                                          <button type="submit" class="btn btn-primary">Add Case Type</button>
                                      </center>
                                  </form>
        ...
        <div class="row" v-if="this.$store.state.cases.all_case_type.length">

        <v-client-table :data="this.$store.state.cases.all_case_type" :columns="case_type_table_columns" :options="this.case_type_table_options" class="table table-striped custom-table">

              <template v-slot:action="item">

                  <button class="btn btn-success btn-xs" data-toggle="modal" :data-target="`#edit_case_type_${ item.row['id'] }`" title="Edit"><i class="fa fa-edit"></i></button>

                  <button class="btn btn-danger btn-xs" @click="deleteCaseType(item.row['id'])" title="Edit"><i class="fa fa-remove"></i></button>

                  <div class="modal fade" :id="`edit_case_type_${ item.row['id'] }`">
                      <div class="modal-dialog" role="document">
                          <div class="modal-content">
                              <div class="modal-header">
                                  <h5 class="modal-title">Update Case Type</h5>
                                  <button type="button" class="close" data-dismiss="modal"><span>&times;</span>
                                  </button>
                              </div>
                              <form method="POST" @submit.prevent="editCaseType(item.row['id'])">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label>Name: </label>
                                                <input type="text" class="form-control" placeholder="" v-model="item.row['name']" :id="`type_name_${ item.row['id'] }`">
                                            </div>
                                        </div>
                                    </div>
...
<script>
    ...
    export default {
        ...
        async fetch() {
            ...
            let case_type = this.$route.params.type;

            let res = await this.$store.dispatch('cases/getAllCaseType', case_type);
